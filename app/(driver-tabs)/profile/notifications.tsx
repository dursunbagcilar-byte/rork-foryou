...
const RIDE_NOTIFS: NotifSetting[] = [
  { key: 'newRideRequest', label: 'Yeni Yolculuk Talebi', desc: 'Yeni bir yolculuk talebi geldiğinde', icon: Navigation, color: Colors.light.primary, iconBg: 'rgba(245,166,35,0.12)' },
  { key: 'rideCancel', label: 'Yolculuk İptali', desc: 'Müşteri yolculuğu iptal ettiğinde', icon: Navigation, color: Colors.light.accent, iconBg: 'rgba(231,76,60,0.12)' },
  { key: 'rideComplete', label: 'Yolculuk Tamamlandı', desc: 'Yolculuk başarıyla tamamlandığında', icon: Navigation, color: Colors.light.success, iconBg: 'rgba(46,204,113,0.12)' },
];

const EARNING_NOTIFS: NotifSetting[] = [
  { key: 'dailyEarnings', label: 'Günlük Kazanç Özeti', desc: 'Her gün sonunda kazanç özetiniz', icon: DollarSign, color: Colors.light.primary, iconBg: 'rgba(245,166,35,0.12)' },
  { key: 'weeklyReport', label: 'Haftalık Rapor', desc: 'Haftalık performans ve kazanç raporu', icon: DollarSign, color: '#3498DB', iconBg: 'rgba(52,152,219,0.12)' },
];

const OTHER_NOTIFS: NotifSetting[] = [
  { key: 'newRating', label: 'Yeni Değerlendirme', desc: 'Müşteri sizi değerlendirdiğinde', icon: Star, color: '#F39C12', iconBg: 'rgba(243,156,18,0.12)' },
  { key: 'hotzone', label: 'Yoğun Bölge Uyarısı', desc: 'Yakınızda yoğun talep olduğunda', icon: MapPin, color: Colors.light.secondary, iconBg: 'rgba(255,107,53,0.12)' },
];
...
export default function DriverNotificationsScreen() {
  ...
  const renderToggleSection = (items: NotifSetting[]) => (
    ...
    <View style={styles.container}>
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          ...
          {renderToggleSection(RIDE_NOTIFS)}

          <Text style={styles.sectionTitle}>Kazanç Bildirimleri</Text>
          {renderToggleSection(EARNING_NOTIFS)}

          <Text style={styles.sectionTitle}>Diğer Bildirimler</Text>
          {renderToggleSection(OTHER_NOTIFS)}

          <Text style={styles.sectionTitle}>Güvenlik Uyarıları</Text>
          {renderToggleSection(SECURITY_NOTIFS)}
...
const styles = StyleSheet.create({
  ...
  bannerTitle: { fontSize: 18, fontWeight: '700' as const, color: Colors.light.text, marginBottom: 4 },
